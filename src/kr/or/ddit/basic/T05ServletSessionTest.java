...
public class T05ServletSessionTest extends HttpServlet {
	 ...
	 * 
	 * - 세션 객체를 이용하여 사용자 (웹브라우저) 별로 구분하여 정보를 관리할 수 있다. (세션 ID 이용) - 쿠키를 사용할 때보다 보안이
	 * 향상된다 (정보가 서버에 저장되기 때문에...)
	 * 
	 * - 세션객체를 가져오는 (생성하는) 방법 HttpSession session = req.getSession(boolean값);
	 * boolean값 : true인 경우 => 세션 객체가 존재하지 않으면 새로 생성하여 반환한다. false인 경우 => 세션 객체가 존재하지
	 * 않으면 null을 리턴한다.
	 * 
	 * - 세션객체를 삭제처리 하는 방법
