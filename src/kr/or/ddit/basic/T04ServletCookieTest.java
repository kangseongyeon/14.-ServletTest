...
public class T04ServletCookieTest extends HttpServlet {
	 ...
	 * 
	 * 1. 구성 요소? - 이름 - 값 - 유효시간(초) - 도메인 : ex) www.somehost.com, .somehost.com =>
	 * 쿠키의 도메인이 쿠키를 생성한 서버의 도메인을 벗어나면 브라우저는 쿠키를 저장(생성)하지 않는다. (보안위험 때문에...) - 경로 :
	 * 쿠키를 공유할 기준경로를 지정한다 (도메인 이후 부분으로 디렉토리 수준) => 지정하지 않으면 URL 경로 부분이 사용된다
	 * 
	 * 2. 동작 방식 - 쿠키 생성 단계 : 생성한 쿠키를 응답 메시지의 헤더에 저장하여 웹브라우저(사용자)에 전송한다 - 쿠키 저장 단계 :
	 * 웹브라우저는 응답데이터에 포함된 쿠키를 쿠키저장소에 보관한다 - 쿠키 전송 단계 : 웹브라우저는 저장한 쿠키를 요청이 있을 때마다 웹서버에
	 * 전송한다 (삭제되기 전까지..) 웹서버는 브라우저가 전송한 쿠키를 사용해서 필요한 작업을 수행한다
	 */
	...
	private void setCookieExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		out.print("<!DOCTYPE html><html><head><title>" + title + "</title></head>" + "<body>"
				+ "<h1 align = \"center\">" + title + "</h1>" + "<ul>" + "<li><b>ID : </b>" + req.getParameter("userId")
				+ "</li>" + "<li><b>이름 : </b>" + req.getParameter("name") + "</li>" + "</ul></body></html>");
